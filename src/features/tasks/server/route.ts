...
const app = new Hono()
  .get(
    ...
    async (c) => {
      ...
      const { workspaceId, assigneeId, dueDate, projectId, search, status } = c.req.valid("query")
  ...
  .post("/", sessionMiddleware, zValidator("json", createTaskSchema), async (c) => {
    ...
    const { name, status, workspaceId, assigneeId, projectId, dueDate } = c.req.valid("json")
  ...
  .patch(
    ...
    async (c) => {
      ...
      const { taskId } = c.req.param()
      const { name, status, description, assigneeId, projectId, dueDate } = c.req.valid("json")
